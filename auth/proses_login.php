...
try {
    ...
    if ($user && verify_user_password($password, $user['password'])) {
        ...
        if (!password_verify($password, $user['password'])) {
            ...
            mysqli_stmt_bind_param($stmtUp, 'si', $newHash = hash_user_password($password), $user['id_user']);
    ...
    error_log('proses_login DB error: ' . $e->getMessage());
    set_flash('Terjadi kesalahan koneksi database. Hubungi administrator.', 'danger');
